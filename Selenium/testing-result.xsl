...
<xsl:stylesheet version="2.0"
	...
	xmlns:math="http://exslt.org/math" xmlns:testng="http://testng.org">
	...
	<xsl:variable name="testDetailsFilter"
		select="if ($testNgXslt.testDetailsFilter) then
...
testng:getVariableSafe($testNgXslt.chartScaleFactor, 1))" />
	<xsl:template name="writeCssFile">
		<xsl:result-document
			href="{testng:absolutePath('style.css')}" format="text">
			<xsl:choose>
...
0px; }
 .testMethodsTable .testMethodStatusCONF td.firstMethodCell { border-left: 5px solid gray; }
	...
	<xsl:template name="writeJsFile">
		<xsl:result-document
			href="{testng:absolutePath('main.js')}" format="text">
 <![CDATA[
...
viewer for your browser.');
 ...
 chartContainer.appendChild(message);
	...
	<xsl:function name="testng:getVariableSafe">
		...
		<xsl:value-of
			select="if ($testVar) then $testVar else $defaultValue" />
	</xsl:function>
	<xsl:function name="testng:trim">
		<xsl:param name="arg" />
		<xsl:sequence
			select="replace(replace($arg,'\s+$',''),'^\s+','')" />
	</xsl:function>
...
$testCaseElement/@name, '.html'))" />
	...
	<xsl:function name="testng:concatParams">
		...
		<xsl:variable name="outputString">
			<xsl:value-of separator=","
				select="for $i in ($params) return $i" />
		</xsl:variable>
	...
	<xsl:function name="testng:testMethodStatus">
		...
		<xsl:variable name="status"
			select="$testMethodElement/@status" />
		<xsl:variable name="statusClass"
			select="concat('testMethodStatus', $status)" />
		<xsl:value-of
			select="if ($testMethodElement/@is-config) then concat($statusClass, '
...
count($testCaseElement/class/test-method[(@status=$state) and (not(@is-config))])" />
	...
	<xsl:function name="testng:suiteStateClass">
		...
		<xsl:value-of
			select="if (count($testCaseElements/class/test-method[(@status='FAIL') and (not(@isconfig))]) > 0) then 'suiteStatusFail' else 'suiteStatusPass'" />
	</xsl:function>
	...
	<xsl:function name="testng:formatDuration">
		...
		<xsl:if test="$durationMs > 86400000">
			<xsl:value-of
				select="format-number($durationMs div 86400000, '#')" />
			d
		</xsl:if>
		<!--Hours -->
		<xsl:if
		...
		<xsl:if test="$durationMs &lt; 86400000">
			<!--Minutes -->
			<xsl:if
				test="($durationMs > 60000) and ($durationMs mod 3600000 > 1000)">
				<xsl:value-of
			...
			<!--Seconds -->
			<xsl:if
				test="($durationMs > 1000) and ($durationMs mod 60000 > 1000)">
				<xsl:value-of
					select="format-number(($durationMs mod 60000) div 1000, '#')" />
				s
			</xsl:if>
		</xsl:if>
	...
	<xsl:function name="testng:isFilterSelected">
		...
		<xsl:value-of
			select="contains($testDetailsFilter, $filterName)" />
	</xsl:function>
	...
	<xsl:template match="/testng-results">
		...
		<xsl:variable name="suiteElements"
			select="if (suite/@url) then document(suite/@url)/suite else
suite" />
		<xsl:call-template name="navigationFile">
			<xsl:with-param name="suiteElements"
				select="$suiteElements" />
			<xsl:with-param name="reporterOutputElement"
				select="reporter-output" />
		</xsl:call-template>
		<!--TODO: Review this -->
		<xsl:result-document
			href="{testng:absolutePath('overview-chart.svg')}" format="xml">
			<svg xmlns="http://www.w3.org/2000/svg"
				preserveAspectRatio="xMidYMid meet" width="{$chartWidth}"
				height="{$chartHeight}" viewBox="0 0 900 300">
				<defs>
					<style type="text/css">
 <![CDATA[
...
font-family:Arial; }
 ...
 .values { fill:black; stroke:none; text-anchor:middle; fontsize:12px; font-weight:bold; }
				...
				<svg id="graphzone" preserveAspectRatio="xMidYMid meet" x="0"
					y="0">
...
'###%')" />
					...
					<xsl:variable name="failedX"
						select="$radius * math:cos($failedAngle)" />
...
'###%')" />
					...
					<xsl:variable name="skipX"
						select="$radius * math:cos($skipAngle)" />
					<xsl:variable name="skipY"
						select="-1 * $radius * math:sin($skipAngle)" />
					<xsl:variable name="skipArc"
						select="if ($skipAngle >= $pi) then 1 else 0" />
					<rect style="fill:red;stroke-width:1;stroke:black;" x="10"
						y="86" width="20" height="20" />
...
{round($failedY_text div 4)})">
								<path
									d="M 0 0 h {$radius} A {$radius},{$radius} 0,{$failedArc},0 {$failedX},{$failedY}
z" />
							</g>
						</xsl:if>
						<xsl:variable name="passRotation"
							select="(($failedCount + $skippedCount) div
$totalCount) * 360" />
		...
		<xsl:result-document
			href="{testng:absolutePath('overview.html')}" format="xhtml">
							...
							<xsl:for-each select="$testCaseElements">
								<tr style="background-color: #f5f5f5; font-size: 12px;">
									...
									<td align="center" style="font-weight: bold;">
										<xsl:value-of
											select="if (testng:testCaseMethodsCount(., '*') > 0) then formatnumber(testng:testCaseMethodsCount(., 'PASS') div testng:testCaseMethodsCount(., '*'), '###%') else
'100%'" />
									</td>
									<xsl:if
										test="compare($testNgXslt.showRuntimeTotals, 'true') = 0">
		...
		<xsl:result-document
			href="{testng:absolutePath('reporterOutput.html')}" format="xhtml">
			<html xmlns="http://www.w3.org/1999/xhtml">
				<xsl:call-template name="htmlHead" />
				<body>
	...
	<xsl:template name="navigationFile">
		...
		<xsl:result-document
			href="{testng:absolutePath('navigation.html')}" format="xhtml">
			<html xmlns="http://www.w3.org/1999/xhtml">
				<xsl:call-template name="htmlHead" />
				<body>
...
'PASS') div testng:suiteMethodsCount($testCaseElements, '*'), '###%')" />
							...
							<xsl:call-template name="suiteTestCasesLinks">
								<xsl:with-param name="testCases"
									select="$testCaseElements" />
							</xsl:call-template>
							<xsl:call-template
								name="suiteTestCasesContentFiles">
								<xsl:with-param name="testCases"
									select="$testCaseElements" />
							</xsl:call-template>
						</xsl:for-each>
...
test" />
		<xsl:result-document
			...
			<html>
				...
				<body>
					<table width="100%"
						style="font-size: 16px; margin-bottom: 10px;" cellspacing="1">
						<tr>
							<td width="100%">
					...
					<xsl:call-template name="testMethods">
						<xsl:with-param name="classes"
							select="$testCaseElements/class" />
						<xsl:with-param name="failedMethods"
							select="$testCaseElements/class/testmethod[@status='FAIL']" />
						<xsl:with-param name="passedMethods"
							select="$testCaseElements/class/testmethod[@status='PASS']" />
						<xsl:with-param name="skipedMethods"
							select="$testCaseElements/class/testmethod[@status='SKIP']" />
					</xsl:call-template>
					<xsl:call-template name="powered-by" />
				</body>
			</html>
	...
	<xsl:template name="testMethods">
		...
		<div style="width: 200px;">
			<label for="groupMethodsCheckBox"
			...
			<br />
			<label for="methodsFilter_ALL"
				style="font-weight: bold; margin: 0;">
				<input id="methodsFilter_ALL" type="checkbox"
					onclick="testMethodsFilterChanged(this,
'ALL')">
					<xsl:if
						test="testng:isFilterSelected('FAIL') = 'true' and testng:isFilterSelected('PASS') = 'true'
...
'CONF')">
		...
		<div id="testMethodsByStatus">
			...
			<table class="testMethodsTable" cellpadding="0" cellspacing="0">
				...
				<xsl:call-template name="testMethodsList">
					<xsl:with-param name="methodList"
						select="$failedMethods" />
					<xsl:with-param name="category"
						select="'byStatus_failed'" />
				</xsl:call-template>
				<xsl:call-template name="testMethodsList">
					<xsl:with-param name="methodList"
						select="$passedMethods" />
					<xsl:with-param name="category"
						select="'byStatus_passed'" />
				</xsl:call-template>
				<xsl:call-template name="testMethodsList">
					<xsl:with-param name="methodList"
						select="$skipedMethods" />
					<xsl:with-param name="category"
						select="'byStatus_skiped'" />
				</xsl:call-template>
			</table>
		</div>
		...
		<div id="testMethodsByClass">
			...
			<xsl:for-each select="$classes">
				<xsl:sort order="ascending" select="@name" />
				<table class="testMethodsTable" cellpadding="0"
					cellspacing="0">
					<tr>
						<td colspan="4">
					...
					<xsl:call-template name="testMethodsList">
						...
						<xsl:with-param name="sortByStartTime"
							select="'true'" />
					</xsl:call-template>
				</table>
				<br />
	...
	<xsl:template name="testMethodsList">
		...
		<xsl:for-each select="$methodList">
			<xsl:sort order="ascending"
				select="if (compare($sortByStartTime, 'true') = 0) then @started-at
else ''" />
			<xsl:variable name="methodId"
				select="concat(../@name, '_', @name, '_', $category, '_',
@status, position())" />
			<xsl:variable name="detailsId"
				select="concat($methodId, '_details')" />
			<xsl:variable name="exceptionDetailsId"
				select="concat($methodId, '_exception')" />
			<tr id="{concat($methodId, '_row')}"
				class="{testng:testMethodStatus(.)}">
...
testng:trim(testng:concatParams(./params/param)), ')')" />
				...
				<td nowrap="true">
					<xsl:if test="./exception">
						<a onclick="toggleDetailsVisibility('{$exceptionDetailsId}')">
							<xsl:value-of
								select="concat(exception/@class, ': ', exception/message)" />
						</a>
					</xsl:if>
					&#160;
				</td>
							...
							<xsl:choose>
								<xsl:when test="exception/full-stacktrace">
									<pre style="padding: 5px; margin: 0;">
										<xsl:value-of
											select="testng:trim(exception/full-stacktrace)" />
									</pre>
								</xsl:when>
								<xsl:when
	...
	<xsl:template name="suiteTestCasesLinks">
		...
		<xsl:for-each select="$testCases">
			<xsl:sort order="ascending"
				select="if (compare($testNgXslt.sortTestCaseLinks, 'true') = 0) then
@name else ''" />
...
'PASS')" />
						...
						<xsl:call-template name="testMethods">
							...
							<xsl:with-param name="passedMethods"
								select="./class/testmethod[@status='PASS']" />
							<xsl:with-param name="skipedMethods"
	...
	<xsl:template name="powered-by">
		<div
			...
			Generated with
			<a href="http://code.google.com/p/testng-xslt/"
				style="color: #8888aa;" target="_blank">
				TestNG XSLT
			</a>
		</div>
	</xsl:template>
